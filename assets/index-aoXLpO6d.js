document.addEventListener("DOMContentLoaded", () => {
    ...
    }, (_, index) => `assets/img${index + 1}.jpeg`);
    const trail = [];
    let mouseX = 0
      , mouseY = 0
      , lastMouseX = 0
    ...
    const setInitialMousePos = (event) => {
        ...
        document.removeEventListener("mouseover", setInitialMousePos, !1)
    }
    ;
    document.addEventListener("mouseover", setInitialMousePos, !1);
    ...
    const removeOldImages = () => {
        const now = Date.now();
        if (now - lastRemovalTime < config.removlalDelay || trail.length === 0)
            return;
    ...
    const splitText = new SplitText(bannerHeader,{
        type: "words"
    });
    ...
    const tooltipSelectors = [{
        trigger: 0.65,
        elements: [".tooltip:nth-child(1) .icon ion-icon", ".tooltip:nth-child(1) .title .line > span", ".tooltip:nth-child(1) .description .line > span", ],
    }, {
        trigger: 0.85,
        elements: [".tooltip:nth-child(2) .icon ion-icon", ".tooltip:nth-child(2) .title .line > span", ".tooltip:nth-child(2) .description .line > span", ],
    }, ];
    ...
    function setupModel() {
        ...
        model.position.set(isMobile ? modelSize.x * 0.82 : -center.x - modelSize.x * 0.4, -center.y + modelSize.y * 0.085, -center.z);
    ...
    loader.load("assets/beer_can.glb", (gltf) => {
        model = gltf.scene;
        model.traverse( (node) => {
    ...
    textContainers.forEach( (container) => {
        const textElements = container.querySelectorAll(".menu-link a, .menu-tag a, .menu-col p");
        let containerSplits = [];
    ...
    const container2 = document.querySelector(".hero3");
